...
class Intent_Slot_Data(object):
    ...
    def __init__(self, train_path, dev_path, test_path, batch_size, flag,max_length,use_auto_bucket):
        ...
        if flag == "train_new":
            ...
            pickle.dump(self.vocab, open(PATH+"/save_model/vocab.p", 'wb'))  # 词典
            pickle.dump(self.slot_vocab, open( PATH+"/save_model/slot_vocab.p", 'wb'))  # 词典
            pickle.dump(self.intent_vocab, open(PATH+"/save_model/intent_vocab.p", 'wb'))  # 词典
        elif flag == "test" or flag == "train":
            self.vocab = pickle.load(open(PATH+"/save_model/vocab.p", 'rb'))  # 词典
            self.slot_vocab = pickle.load(open(PATH+"/save_model/slot_vocab.p", 'rb'))  # 词典
            self.intent_vocab = pickle.load(open(PATH+"/save_model/intent_vocab.p", 'rb'))  # 词典
        else:
            pass
        self.vocab_num=len(self.vocab)
        self.slot_num=len(self.slot_vocab)
        self.intent_num=len(self.intent_vocab)
        self.data_dict= self.data_deal_train()
    ...
    def padd_sentences_no_buckets(self, sent_list):
        ...
        for sent, slot,intent in zip(words, slot_labels,intent_labels):
            ...
            intent_normal=np.array(intent_list)
    ...
    def data_deal_train(self):
        ...
        word_arr, slot_arr, intent_arr, real_len_arr, = self.padd_sentences_no_buckets(data_list)
        data_dict=defaultdict(list)
        for i in range(intent_arr.shape[0]):
            intent=intent_arr[i][0]
            ll={'word_arr':word_arr[i],'real_len_arr':real_len_arr[i]}
            data_dict[intent].append(ll)

        return data_dict

    def get_one_sample(self,pos_k,neg_k):

        pos_k_list=self.data_dict[pos_k]
        neg_k_list=self.data_dict[neg_k]

        random.shuffle(pos_k_list)
        pos_0=pos_k_list[0]
        pos_1=pos_k_list[1]
        random.shuffle(neg_k_list)
        neg_0=neg_k_list[0]

        return {'pos_0':{'word_arr':pos_0['word_arr'],'real_len_arr':pos_0['real_len_arr'],'intent':pos_k},
                 'pos_1':{'word_arr':pos_1['word_arr'],'real_len_arr':pos_1['real_len_arr']},
                'neg_0':{'word_arr':neg_0['word_arr'],'real_len_arr':neg_0['real_len_arr']},
    ...
    def get_train_data(self):
        ...
        for k in keys:
            other_key=keys[:].remove(k)

        for _ in range(15000):
            random.shuffle(keys)
            pos_k,neg_k=keys[0],keys[1]
            ele=self.get_one_sample(pos_k,neg_k)
    ...
    def get_dev_data(self):
        ...
        for _ in range(2000):
            ...
            ele= {'pos_0': {'word_arr': pos_0['word_arr'], 'real_len_arr': pos_0['real_len_arr'],'intent':pos_k},
                    'pos_1': {'word_arr': pos_1['word_arr'], 'real_len_arr': pos_1['real_len_arr']},
                    'neg_0': {'word_arr': neg_0['word_arr'], 'real_len_arr': neg_0['real_len_arr']}
                    }

            dev_datas.append(ele)
        return dev_datas

    def get_dev_data_new(self):
        ...
        return word_arr,real_len_arr,intent_arr

    def get_dev_data_dict(self):
        train_flie = open(self.dev_path, 'r')
        data_list = [line for line in train_flie.readlines()]
        data_list.sort(key=lambda x: len(x))  # sort not shuffle
    ...
    def get_norm_array(self,data_dict,n):
        '''
        获取标准句的矩阵
        :param data_dict:
        :param n:
        ...
        return pos_1_array,pos_1_len,pos_1_label
...
if __name__ == '__main__':
    ...
    id2sent=dd.id2sent
    train_data=dd.train_data
    dev_data=dd.get_dev_data()

    dd.get_norm_array(dd.get_train_data_dict(),3)
