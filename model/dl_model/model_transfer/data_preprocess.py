...
class Intent_Slot_Data(object):
    ...
    intent_slot 数据处理模块
    '''
    def __init__(self, train_path, dev_path, test_path, batch_size, flag,max_length,use_auto_bucket,save_model):
        ...
        if flag == "train_new":
            ...
            pickle.dump(self.vocab, open(PATH+"/save_model/vocab_%s.p"%save_model, 'wb'))  # 词典
            pickle.dump(self.slot_vocab, open( PATH+"/save_model/slot_vocab_%s.p"%save_model, 'wb'))  # 词典
            pickle.dump(self.intent_vocab, open(PATH+"/save_model/intent_vocab_%s.p"%save_model, 'wb'))  # 词典
        elif flag == "test" or flag == "train":
            self.vocab = pickle.load(open(PATH+"/save_model/vocab_%s.p"%save_model, 'rb'))  # 词典
            self.slot_vocab = pickle.load(open(PATH+"/save_model/slot_vocab_%s.p"%save_model, 'rb'))  # 词典
            self.intent_vocab = pickle.load(open(PATH+"/save_model/intent_vocab_%s.p"%save_model, 'rb'))  # 词典
        else:
            pass
